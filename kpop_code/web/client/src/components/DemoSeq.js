...
export default function DemoSeq() {
  const navigate = useNavigate();

  const jobId = '141ad33ee1b44ca292f55f359ec39d7f';
  const finalVideo = `/data/${jobId}/rendered.mp4`;
  const feedbackJsonUrl = `/data/${jobId}/sequence_feedback.json`;

  const [seqFeedbacks, setSeqFeedbacks] = useState([]);
  ...
  useEffect(() => {
    fetch(feedbackJsonUrl)
      .then((res) => res.json())
      .then((data) => setSeqFeedbacks(data))
      .catch((err) => {
        console.error('시연용 JSON 로딩 실패:', err);
        alert('시연용 피드백 데이터를 불러올 수 없습니다.');
      });
  }, []);
  ...
  const handleTimeUpdate = () => {
    ...
    if (matched) {
      shownRef.current.add(matched.start_time);
  ...
  const downloadVideo = () => {
    ...
    setTimeout(() => {
      ...
      a.href = finalVideo;
      a.download = 'dance_feedback_demo.mp4';
      a.click();
    }, 2000);
  };
  ...
  const downloadJSON = () => {
    ...
    setTimeout(() => {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = 'feedback_report_demo.json';
      a.click();
